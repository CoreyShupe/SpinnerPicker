...
window**. Spin for a random pick that skips whatever came up in the last
...
a score for each user on the current round, and read all-time totals. Only the
latest round is editable — spinning again starts a new round and locks the
previous one.
...
Open the printed frontend URL, create a wheel, add a few options, and spin.
...
most recently picked options, then makes a **uniform** random choice among the
rest. If _N_ is ≥ the number of options, it's automatically clamped so at least
one option is always eligible. Selection and history recording happen in a single
SQLite transaction. See
...
| `PATCH`  | `/api/options/:id`            | Edit label / color                   |
