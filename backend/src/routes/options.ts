...
import { asObject, optionalString, parseId, requireString } from '../lib/validation.js';
...
optionsRouter.post('/wheels/:wheelId/options', async (c) => {
  ...
  const option = optionsRepo.create({ wheelId, label, color });
...
optionsRouter.patch('/options/:id', async (c) => {
  ...
  if (label === undefined && color === undefined) {
    throw ApiError.badRequest('Provide at least one of: label, color');
  }

  const updated = optionsRepo.update(id, { label, color });
